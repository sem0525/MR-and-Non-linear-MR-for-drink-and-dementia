...
for(snpi in drink_95snp_name){
  ...
  result <- cbind(snpi,R2_overall)
  r2_overall_result <- rbind(r2_overall_result, result)
}
modeldata$IV <- modeldata$wPRS_95
snpi <- "Alcohol-GS"
result <- cbind(snpi,R2_overall)
r2_result <- rbind(r2_result, result)
R2_overall <- r2_result
