...
plot_nlmr <- ggplot(f_data, aes(x = X)) +
  ...
  scale_y_continuous(name="Hazard ratio (95% CI)",
                     expand = c(0,0),
                     limits = c(0,ylimit,200),
                     breaks = c(1,seq(400,2010,400))
                     #n.breaks = 6
  ) +
...
covar_f_name <- c("age","area","array",paste0("component",1:40))
x_prs_name <- "wPRS_95"
ex_name <- "Exposure"
y_name <- "outcome"
y_time <- "years"
...
model <- with(summ_data$summary, frac_poly_summ_mr(by, bx, byse, bxse, xmean, 
                                                   ...
                                                   seed=200))
summary(model)
p_allover <- model$coefficients[1,5]
p_quad <- model$p_tests[2]
p_Co <- model$p_tests[4]
#p value
p_quad0 <- p_out_fig(p_quad)
...
f_data$pHR_low <- exp(f_data$lci)
f_data$pHR_up <- exp(f_data$uci)
f_data$pHR_up[f_data$pHR_up>=ylimit] = ylimit
f_data$X <- 10^f_data$x
...
f_data$pHR_up_new[f_data$pHR_up_new>=ylimit] <- ylimit
...
plot_nlmr_ranked_log_pos <- plot_nlmr
plot_nlmr_ranked_log_pos_men <- plot_nlmr_men
plot_nlmr_ranked_log_pos_women <- plot_nlmr_women
